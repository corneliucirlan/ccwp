    ...
    class Tags
    {
        ...
        public static function postedOn()
        {
            ...
            if (get_the_time('U') !== get_the_modified_time('U'))
                $timeString = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated hide" datetime="%3$s">%4$s</time>';
            ...
            $posted_on = sprintf(
                esc_html_x('Posted on %s', 'post date', 'ccwp'),
                '<a href="'.esc_url(get_permalink()).'" rel="bookmark">'.$timeString.'</a>'
            );

            $byline = sprintf(
                esc_html_x('by %s', 'post author', 'ccwp'),
